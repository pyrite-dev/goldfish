...
/* Copy two points */
static COMPACT_25519_DECL inline void f25519_copy(gf_uint8_t* x, const gf_uint8_t* a) { memcpy(x, a, F25519_SIZE); }
...
/* Copy an element */
static COMPACT_25519_DECL inline void fprime_copy(gf_uint8_t* x, const gf_uint8_t* a) { memcpy(x, a, FPRIME_SIZE); }
...
static COMPACT_25519_DECL inline void sha512_init(struct sha512_state* s) { memcpy(s, &sha512_initial_state, sizeof(*s)); }
 ...
 */
static COMPACT_25519_DECL inline void c25519_prepare(gf_uint8_t* key) {
	key[0] &= 0xf8;
	key[31] &= 0x7f;
	key[31] |= 0x40;
}
...
static COMPACT_25519_DECL inline void ed25519_prepare(gf_uint8_t* e) {
	e[0] &= 0xf8;
	e[31] &= 0x7f;
	e[31] |= 0x40;
}
...
static COMPACT_25519_DECL inline void ed25519_copy(struct ed25519_pt* dst, const struct ed25519_pt* src) { memcpy(dst, src, sizeof(*dst)); }
...
static COMPACT_25519_DECL inline int min_int(int a, int b) { return a < b ? a : b; }
...
static COMPACT_25519_DECL inline gf_uint64_t load64(const gf_uint8_t* x) {
	gf_uint64_t r;
...
static COMPACT_25519_DECL inline void store64(gf_uint8_t* x, gf_uint64_t v) {
	x += 7;
	*(x--) = v;
	v >>= 8;
	*(x--) = v;
...
static COMPACT_25519_DECL inline gf_uint64_t rot64(gf_uint64_t x, int bits) { return (x >> bits) | (x << (64 - bits)); }
